...
function Cursor({ visible = true }: { visible?: boolean }) {
  ...
  return (
    <span
      className="inline-block animate-[blink_1s_step-end_infinite] text-white"
      aria-hidden="true"
    >
      █
    </span>
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function V4StarfieldTyping() {
  const [lang, setLang] = useState<Lang>("en");

  const heroName = useTypingEffect(content.name.toUpperCase(), 80, 600);
  const heroTitle = useTypingEffect(content.title[lang], 40, 1200);
    ...
    <div className="min-h-screen text-zinc-100">
      ...
      <div className="relative z-10">
        ...
        <section className="relative flex h-screen w-full flex-col items-center justify-center px-6 text-center">
          ...
          >
            <h1 className="mb-6 font-thin leading-none tracking-[0.08em] text-white text-7xl md:text-9xl">
              {heroName.displayed}
              <Cursor visible={!heroName.done} />
            </h1>
            <div className="mb-4 text-base font-light uppercase tracking-[0.35em] text-zinc-200">
              {heroName.done && (
                <>
                  {heroTitle.displayed}
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.9, ease: "easeOut", delay: 3 }}
