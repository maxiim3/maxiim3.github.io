...
function Cursor({ visible = true }: { visible?: boolean }) {
  ...
  return (
    <span
      className="inline-block animate-[blink_1s_step-end_infinite] text-zinc-100"
      aria-hidden="true"
    >
      █
    </span>
...
// ─── Main component ──────────────────────────────────────────────────────────
export default function V4MonographTyping() {
    ...
    <div className="min-h-screen bg-[#111111] text-zinc-100 antialiased">
      ...
      <header className="relative z-10 flex min-h-screen flex-col justify-between px-6 md:px-12 lg:px-24 py-8">
        ...
        <div className="flex flex-1 flex-col justify-center">
          {/* Name — massive, left-aligned, with typing effect */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.6 }}
          >
            <h1 className="-ml-1 md:-ml-2 font-serif text-[clamp(6rem,18vw,16rem)] font-thin leading-[0.85] tracking-tight text-zinc-100 select-none">
              {heroName.displayed}
          ...
          <div className="mt-8 md:mt-12 max-w-lg md:ml-2">
            <div className="font-sans text-xs font-medium uppercase tracking-[0.35em] text-zinc-400">
              {heroName.done && (
                <>
                  {heroTitle.displayed}
